...
def main():
    ...
    kanji_dir = project_root / 'kanji'
    docs_kanji_dir = project_root / 'docs' / 'kanji'
    ...
    if all_errors:
        ...
        if fixable and args.fix:
            ...
            import subprocess
            subprocess.run(['python3', str(script_dir / 'update_kanji_index.py'), '--rebuild-all'], check=True)
            subprocess.run(['python3', str(script_dir / 'build_kanji_html.py')], check=True)
