...
This pipeline is part of **je-dict-1**, a Japanese-English learner's dictionary targeting intermediate learners who can read kana and are building vocabulary. The live site is at https://www.tkgje.jp/. It is a completely static site with over 13,500 entries as of early 2026.
...
- **General** (10,700+ entries, growing) — all other vocabulary; all new entries go here
...
1. The script selects a batch of **seed words** (default 15) from `entries_and_candidates_for_LLM_brainstorming.json` — a flat JSON array of all current dictionary entries and candidate words, each with `headword`, `reading`, `gloss`, and `checked` fields.
...
5. Survivors are saved to `new_candidates_by_{model}.json` with provenance (which seed inspired each suggestion, which run produced it).
...
1. `entries_and_candidates_for_LLM_brainstorming.json` exists in this directory
...
The output file is a standalone JSON file. In a separate step (not part of this pipeline), candidates from this file can be reviewed and selectively imported into the main dictionary's `candidate_words.json` using `python3 build/manage_candidates.py add`. That import step includes its own duplicate checking.
