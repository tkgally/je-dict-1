...
## 2026-06-12 accuracy-review run 010

[tooling] accuracy-review flag rate was 54.6% (273/500) for range 03301–03800, far above the 20% noise threshold. The primary noise family is "general is too broad/too vague" suggestions — the reviewer model flags any entry with the generic 'general' semantic tag as needing a more specific replacement, even though 'general' is a valid in-list tag per VALID_SEMANTIC. The reviewer prompt should be clarified to distinguish (a) tags not in VALID_SEMANTIC (always fix) vs. (b) overly-generic valid tags (reviewer's preference, ignore). Secondary noise: formality tags ('formal' for neutral words) — many correct fixes, but also many false positives for words whose notes explicitly describe a formal register.
