...
- [pattern] gemini-2.5-flash consistently claims widely-used semantic tags (culture/religion/entertainment/business/nature) are invalid — verified false (all have 100+ uses). Reviewer prompt may benefit from explicit valid-tag list to suppress false positives.

[entry] 06917 zo: formality tag incorrectly set to "formal" (notes say "Casual to rough. Never used in formal situations."); semantic tags include wrong values ("clothing", "time-general", "tool") — wrong for a sentence-final particle. Needs tier-1 fix.

## 2026-06-11 — accuracy-review session 003 (entries 01151–01650)
- [tooling] review_accuracy.py flagged 137/498 entries (27.5% > 20% threshold) — mostly tag "too narrow/too broad" noise. Primary noise families: semantic tags flagged as "too narrow/too broad for headword" (subjective, ~120 flags bulk-rejected); formality "formal" flagged as "neutral" for slightly-formal words (defensible, ~10 flags rejected). Only 11 entries had genuinely applicable fixes (mainly gloss completeness and kimono/以下 translation accuracy). Tag-dimension precision this run: ~9% (11 apply / 121 flags).
- [tooling] Gemini-2.5-flash flags semantic tags as "too narrow/broad" without reference to the valid tag list or project conventions — many suggestions (education→cognition, communication→general, work→action) are subjective preference not factual error. Consider strengthening the accuracy-review prompt to cite project convention: "general, descriptive, action, expression are legitimate fallback tags; do not flag if the current tag is semantically plausible."
