...
- [tooling] score_note_quality.py depresses scores of fully-polished entries, polluting the notes priority list. Two false signals: (1) `has_bare_kanji` counts inline-link baseforms (e.g. the `小さな` in `⟦{小|ちい}さな→小さな：02913_chiisana⟧`) as un-furiganaed kanji, so any entry with inline links in its notes scores furigana=0; (2) the `required_sections` matcher misses valid section headers, giving required=0 to good notes. Result: the top ~30 notes-priority entries (00025_chiisai, 00530_chikai, 00533_osoi, etc.) are already clean and need no changes. The scorer should strip ⟦…⟧ link baseforms before the bare-kanji check. (routine polish 2026-06-18)
- [pattern] The -teki adjective/adverb cluster around 06169-06176 (実質的, 比較的, 定期的, 段階的, 総合的, 保守的) was created in Jan 2026 with zero inline-link coverage in examples or notes — they predate the inline-link polishing step. The sequential frontier is now reaching this Jan-created band; expect most 06000-07000 entries to need full tier-1 inline linking. (routine polish 2026-06-18)

- [pattern] 2026-06-18 (new-entries routine): candidate_words.json (~1287) is dominated by corpus-harvested noise — bare numbers/counters (三歳, 二百, 一万人), non-lexical compound fragments (倍率差, 機成り, 些道, 多角的一面), transparent decomposable compounds (給水槽, 排水処理, 〜率/〜化/〜性 derivatives), and ad-hoc phrases (あらかじめ準備する, 用につき). Genuine standalone learner words are sparse, so a new-entries run that strictly follows "oldest first" would create low-value entries. This run curated 13 quality words instead. Suggest a curator cleanup pass on candidate_words.json (or a heuristic filter in clean_up_candidates_list) to purge counter/number/fragment candidates, and a restock with vetted vocabulary.
