...
## 2026-06-22 — routine polish (priority lane lines 1–62 reviewed; frontier 06231–06236)
- [tooling] Priority `notes.txt` ranking does not account for recent modification: of the first ~62 priority lines, nearly all top entries (e.g. 01118_nai, 02352_iroiro, 00335_ookii) were polished within the last 30 days and were skipped, and all 6 eligible entries I did reach (02848_issho, 02947_hikui, 00025_chiisai, 00533_osoi, 00304_nandemo, 00642_kinyoubi) were already clean. `prioritize_polishing.py` re-ranking won't fix this — the scorer surfaces the same already-polished function/basic words each run. Consider down-weighting (or excluding) entries modified < N days ago in the priority generator so the lane spends budget on genuinely stale low-scorers.
- [pattern] Frontier entries created in the 2026-01 batch (06231–06236) had ZERO inline links in examples or notes despite being otherwise well-formed (good notes, cross-refs, conjugation). Inline-link coverage is the dominant tier-1 gap on the unpolished frontier; budget per frontier entry is high because of dense collocation lists in notes.
- [pattern] Malformed furigana in older notes: 06231 had a non-kanji char inside braces (`{お吸|す}い{物|もの}` → should be `お{吸|す}い{物|もの}`) and nested braces (`{とん{汁|じる}}`, `{けんちん{汁|じる}}`). These pass schema but are wrong; a detector for "kana character inside a furigana kanji group" / "nested braces" would catch them.
- [entry] 00642_kinyoubi (Friday): has no cross_references; could link to the other six weekday entries (related). Left for a future session to avoid a 6-entry back-link cascade.
