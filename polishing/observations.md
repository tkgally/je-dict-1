...
- [tooling] check_example_headword.py noise families (2026-06-14 routine, example-headword-missing item): after the clean-entry frontier is exhausted, the detector's remaining flags are dominated by (1) U+FFFD-corrupted entries (kanji headword is mojibake, so it can never match — owned by tooling-backlog #16) and (2) legitimate false positives: kana/katakana orthography of the headword (ごちそう, ヤギ, シミ, カツオ, カキ, しわ寄せ), documented compound forms (退職届 in 届け), the radical sense (うかんむり in 冠), and ～-prefix headwords like 〜時 whose plain-kanji examples the matcher can't bind to the tilde'd headword string. Worth teaching the detector to (a) skip entries containing U+FFFD, (b) strip a leading ～/〜 from the headword before matching, and (c) treat katakana of the reading as a headword match for animal/seafood nouns — would cut this queue from ~31 entries to near zero.
