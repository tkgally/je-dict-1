...
[tooling] 2026-06-11 accuracy-review pass on entries 1651-2100: 37% of entries flagged (166/444), well above the 20% noise threshold. The high rate is driven by: (a) many semantic tags not in VALID_SEMANTIC (people/abstract/nature/object/place/daily-life/media/health etc.) — these were real errors applied across many entries, all fixed this session; (b) excessive formality flag disputes (the reviewer marks many neutral words as formal/informal); (c) 'too narrow tag' complaints where the original tag IS valid. Recommend reviewing the accuracy reviewer prompt to suppress formality flags when the existing tag is reasonable, and to only flag semantic tags actually invalid per VALID_SEMANTIC.
