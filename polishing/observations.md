...
- [tooling] accuracy-review (gloss/translation/tags) via gemini-2.5-flash flagged 246/492 entries (50%) — far above the 20% noise threshold. Breakdown: 252 tag flags, 19 translation flags, 14 gloss flags. The dominant pattern (88% of tag flags) is the model flagging the `general` semantic tag as "too broad" — it does not understand that `general` is a valid catch-all in this dictionary's taxonomy. The reviewer prompt should be updated to explicitly instruct the model that `general` is a valid semantic tag meaning "does not fit a more specific category" and should NOT be flagged as too broad. Secondary pattern: the model flags formality/politeness tags using its own intuitions rather than the dictionary's defined tag meanings. All 285 flags were REJECT-logged in decisions.jsonl. Genuine fixes this run: 2 furigana corrections (03884, 04162) found via screening pass (0% false positive rate for screening vs 88% for deep accuracy review on tags).

## 2026-06-13 — polish routine, session 008 (entries 06101–06110 + priority 01327, 04981)
- [pattern] Semantic tag `transportation` incorrectly applied to 06107_junshu (遵守, compliance/observance). This suggests the 06000-range may have other misassigned tags from the same batch creation pass. Worth running `check_tag_drift.py` or manually auditing 06000–06200 for entries with domain tags inconsistent with their gloss/semantic field.
- [pattern] 06101_hakumai (白米) had tag `general` instead of `food`; 06106_katsuo (鰹) had tag `general` instead of `animal-fish`. Food/ingredient entries in this range may have defaulted to `general` during batch creation rather than getting domain-appropriate semantic tags.
- [entry] 06109_karorii (カロリー, calorie) — encountered but not processed this session. Likely needs inline links added to examples and notes.
