...
- [pattern] candidate_words.json quality is low outside the "seen in entry" set: the Feb–May 2026 corpus/brainstorm batches are dominated by compositional compounds, numbers, dialect fragments, place names, and outright errors (e.g. 怒燥 for 怒涛, アンパッサン glossed "ice cream sundae"). new-entries runs find few genuinely useful headwords beyond the seen-in-entry candidates (only 5 this run). Curator restock with higher-quality, common, learner-relevant words would raise the value of new-entries runs. (seen during routine new-entries 2026-06-18)
