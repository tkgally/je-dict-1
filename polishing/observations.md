...
## Routine accuracy-review 2026-06-21_005 (range 8038–8237, phase furigana)

- [pattern] **Cleanup P20 free-form-tag cohort confirmed unbroken into 8038–8237, denser than 7815–8037**: deterministic `VALID_SEMANTIC` scan found 177/200 entries (88%) with out-of-list semantic tags, 305 instances across 162 distinct tags. Continues contiguously from the 7815–8037 block (73%). Migrated a 14-entry hand-verified near-duplicate slice; deferred ~240-instance long tail to the queued 7000–8500 systemic-fix sweep (now scope ≥7815–8237). Backlog P20 updated.
- [pattern] **Furigana screener ~0% precision over this structured cohort, again**: all 16 screening flags in 8038–8237 were the documented false-positive families (rendaku in compounds まとめ買い/やけ食い, okurigana/compound-furigana splits, the 美術館 encoding-artifact). Deep pass surfaced 2 genuine malformed-grouping errors only ({全部食|ぜんぶた}→{全部|ぜんぶ}{食|た}, {メガ盛|も}→メガ{盛|も}) — both *grouping* errors the screener does NOT flag, not reading errors. Reinforces Tooling item 24 (screener known-noise) and suggests a cheap deterministic "multi-token-under-one-furigana-with-leading-katakana/okurigana-orphan" detector would catch the grouping errors the LLM screener misses.
- [tooling] review_accuracy `tags` dimension occasionally hallucinates tags not present in the entry on a first read but was reliable here; the genuine value this run was (a) catching one of my own weak migrations (飲みごたえ satisfaction→emotion, correctly flagged for removal) and (b) the gloss/translation pass catching 指揮棒を振った "raised"→"waved". Translation/self-check dimensions earning their keep; tags dimension remains dominated by the P20 systemic family.

## Routine 2026-06-21 (polish mode)

- [tooling] `score_note_quality.py` penalizes well-formed adjective/particle/noun notes that use descriptive section headers (TWO MEANINGS, FORMS, DEGREES OF LIKING, PATTERN 1/2/3) instead of the literal "usage"/"functions" keyword the POS template requires. Many recently-polished basic-tier entries score 50–58 despite being complete and correctly formatted (00025 chiisai, 00533 osoi, 02355 suki, 00765 yasashii, 02006 bakari, 02007 mama). The "notes" priority list is therefore dominated by false positives. Consider broadening `find_sections` variant matching (e.g. treat any header + bulleted pattern list as satisfying "usage"/"functions") so the priority ranking tracks real note quality.
- [pattern] Frontier band ~06222–06230 (and likely a wider range of similarly-aged general-tier entries) was created with ZERO inline links in examples and `・` (nakaguro) bullets in notes. Comprehensive-polish frontier lane is fixing these one block at a time; this band needs continued sequential sweeping.

- [tooling] check_artifacts.py missing-target-id now converges: it only flags refs whose referenced word actually has an entry (resolvable now). Refs to no-entry words (homophone/antonym/contrast display labels, transitivity-pair pointers awaiting an entry) are intentional and no longer re-surfaced every run. `--include-intentional` restores the full audit list (77 as of 2026-06-21). This implements the recommendation logged by the 2026-06-19 and 2026-06-20 routine runs. (routine 2026-06-21)
- [pattern] artifact-missing-target-id backlog item is effectively RESOLVED: after filling the 5 newly-resolvable refs (29368–29383 curator restock), 0 resolvable target-less refs remain dictionary-wide; the 77 remaining are intentional pointers the refined detector now excludes by design. (routine 2026-06-21)

- [tooling] 2026-06-21 routine(polish): the notes-priority list (polishing/priority/notes.txt) keeps ranking already-well-polished basic-tier function words (一緒, 小さい, 遅い, 何でも, 金曜日, 隣, 腕, ばかり, まま, 優しい, 好き, 青) at the very top. All 12 eligible (non-recently-modified) priority entries examined this run were fully linked, furigana-complete, and needed zero changes. prioritize_polishing.py's notes scorer appears to penalize short-but-adequate notes on simple basic words, producing false-positive priority. Suggest the scorer weight by entry complexity / tier or exclude entries whose notes already pass a coverage check, so the priority lane surfaces genuinely deficient entries.
