...
## 2026-06-18 — routine v2 accuracy-review (furigana phase, entries 06926–07139), session 007
- [tooling] `review_runner.py --pass deep` aborted after the first entry (06930): queried both openai/gpt-4.1 and google/gemini-2.5-pro, then exited 0 with no result files written and no error logged. The screening pass (gemini-2.5-flash) ran fine for ~214 entries but is throughput-bound (~10 entries/min), so a 518-entry range cannot finish within a 25-min wrapper. Suggest: (a) investigate the deep-pass single-entry abort (likely gemini-2.5-pro timeout/parse failure swallowed silently), and (b) size furigana-screening ranges to ~200/run given the measured rate.
- [tooling] Screening (gemini-2.5-flash) "incomplete reading" flags are dominated by the runner's pair-extraction truncating long compound readings: e.g. 入場料→「にゅうじ」, 電話→「でん」, 電気技師→「でんき」, 観客席→「かんきゃ」 were all flagged as cut-off, but the actual entries hold the correct full readings ({入場料|にゅうじょうりょう} etc.). 39/40 flags this range were false positives (okurigana-split family + extraction truncation + shogi/military reading confusion on 歩兵). Precision ~2.5%, consistent with the documented 0–5% on already-polished ranges. The one true positive (06952) was a genuine non-furigana signal (Latin "uu" leaked into the reading), which a non-hiragana-reading lint would catch far more cheaply than the screener.

- [pattern] 2026-06-19 routine(polish): frontier block 06177–06183 (onomatopoeia adverbs + tech loanwords) had zero inline-link coverage in examples and notes despite some recent `modified` timestamps. Inline-linking polish has not yet reached this block; sequential frontier is doing useful net-new work here.

## 2026-06-19 — routine systemic-fix (missing-target-id)

- [tooling] `check_artifacts.py --issue missing-target-id` over-counts. Of 136 flagged refs, 40 were genuinely resolvable (now fixed → 96 remain). The residual ~96 are mostly **legitimate target-less refs**: the referenced word has no entry, and many are *intentional* homophone/contrast/antonym display labels (e.g. 00250 工夫/こうふ "laborer (homophone)", 17797 侯爵 "marquis (homophone)", 00296 有限 "finite"). Consider teaching the detector to exclude refs whose `type` is `homophone`/`contrast` (or that carry a `label`/`note`) AND whose reading has no entry, so the queue converges instead of flagging permanent intentional pointers.
- [tooling] Latent build mis-resolution: 04026_hatsu's antonym `〜着` (arrival sense, ちゃく) has no entry; the build-time by-reading fallback would resolve it to 27655_chaku (着 = *counter for suits of clothing*), the wrong sense. Left target_id unset. The reading-fallback resolver should require a headword-surface match, not reading-only.
- [pattern] 26 of the 136 dangling refs already carried a vestigial non-schema `id` field (renderer reads `target_id` only). 4 of those `id`s were stale (pre-renumber: 01040_shinjin, 02032_tenkin, 00373_ondo, 00213_gaikokujin). Promoted/repointed all 26 to `target_id`. Newer entry-creation may still emit `id` instead of `target_id` — worth checking the entry-creation skill/templates.

- [pattern] 2026-06-19 routine(polish): all 8 eligible notes-priority entries from line 57 (01092 oku, 02350 yoi, 00642 kinyoubi, 01003 tonari, 01006 ude, 02006 bakari, 02007 mama, 00765 yasashii) were already fully polished (complete inline links, good notes, examples) and needed zero changes. The note-quality ranking (generated 2026-06-18) is stale relative to recent basic/core polishing — regenerated priorities + reset cursor this run. Note-quality scorer may be flagging these on note *length* while they are actually complete.
- [tooling] 2026-06-19 routine(polish): the frontier block ~06184-06189 (created 2026-01-17) has zero inline links in examples or notes — a large pre-inline-link cohort sits between the polish frontier (~06190) and ~07000. These are heavy (full notes+example linking from scratch). Consider a targeted inline-link sweep for the 06000 range.
