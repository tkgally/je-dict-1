...
## 2026-06-19 — routine polish session 006 (priority lane 6 entries; frontier 06190–06196)
- [tooling] The note-quality priority lane (`polishing/priority/notes.txt`) keeps surfacing already-complete basic/core entries at the very top: 6/6 processed (00025 chiisai, 00533 osoi, 00304 nandemo, 01092 oku, 00642 kinyoubi, 01003 tonari) needed zero changes — full inline links, full example sets, thorough notes, cross-refs. The scorer appears to penalize terse-but-complete basic-tier entries (short notes relative to length heuristics) and never lets them off the top, so the priority lane currently does no useful work and risks looping on the same IDs after each regenerate+reset. Recommend the scorer either factor in inline-link completeness / example count / cross-ref presence, or down-weight entries modified within N days, so the priority lane points at genuinely deficient entries.
- [entry] Duplicate entry for 気持ち: both 01385_kimochi (basic) and 02485_kimochi (core) exist with identical headword/reading/gloss. Candidate for consolidation (resolve-duplicates). Linked 06194 to 01385 for now.
- [pattern] The 06190s frontier block (lottery of nouns/proverbs created 2026-01) had NO inline links in examples or notes — confirms the comprehensive frontier still has long stretches of unlinked entries below the current cursor that predate the inline-link requirement.

- [pattern] Several early general-tier entries (e.g. 06198 `{きつい|}` empty-reading wrapper, 06202 `{お年寄|としよ}り` with prefix お wrongly inside the furigana braces) carry malformed furigana from the Jan-2026 creation batch. `build/check_furigana_format.py` should catch these; worth a systemic-fix sweep over the 06000–06400 range. (seen 2026-06-20T03:37:03Z)

- [tooling] (2026-06-20) `check_artifacts.py --issue missing-target-id` has hovered near 80–96 for three systemic-fix runs (190→136→96→82). Each run only drains the handful of refs whose referenced word gained an entry since last run; the residual ~80 are intentional target-less pointers (homophone/contrast/antonym display labels for words with no entry). The detector should exclude `type=homophone`/`contrast` (and explicitly `label`-bearing) refs whose reading has no entry, so the queue reflects only genuinely-fixable dangling refs and converges toward 0 instead of re-flagging permanent pointers every run.

- [pattern] (2026-06-20 routine polish) Linked 06204–06209 (general nouns: 車掌/序文/付録/栄養素/炭水化物/太陽光) — all were schema-valid and furigana-complete but had ZERO inline links in examples and notes. The unlinked-frontier block continues unbroken from ~06190 through at least 06209. The priority/notes.txt lane, by contrast, was 6/6 no-change (basic-tier particles/adjectives already fully linked), so note-quality priority rankings are stale vs. the real gap, which is inline-link coverage on the general-tier frontier.
