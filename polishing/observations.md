...
- [tooling] review_runner.py `--pass deep --range A B` deep-reviews ALL entries in range, not just screening-flagged ones (resolve_entry_ids returns the full range, bypassing get_flagged_entry_ids). For accuracy-review §A intent ("deep covers only flagged"), pass `--ids <flagged>` instead, or the deep pass blows past its time/budget on the whole range. Seen 2026-06-10 routine accuracy-review run (range 451-650: 199 entries instead of 45 flagged).
- [tooling] review_runner deep pass: google/gemini-2.5-pro intermittently returns a bare JSON array instead of the expected object, triggering "Failed to parse response" per-entry; gpt-4.1 still returns valid data so the entry is reviewed single-model and the pass exits 0. Not the null-response crash; just reduced redundancy on those entries (2026-06-10).
- [pattern] accuracy-review reviewer-flag precision on basic/core foundational entries (IDs 451-650) was very low: 6 applied / 413 flags (~1.5%). gemini-2.5-flash over-flags concise basic-tier glosses as "incomplete" and pushes encyclopedic senses, mislabels schema-default politeness/plain tags on nouns, and misreads 何か/何も/どうして compound examples as headword mismatches. Furigana screening false-positive rate similar (45 screened-flagged → 0 genuine after deep review). Tags dimension caught the only real wins (clothing→tool on 傘/かばん, education→grammatical on 修飾).

## routine v2 systemic-fix session (2026-06-10, artifact-teiru-brace)

- [pattern] Furigana self-check (§4 screener variant, gemini-2.5-flash) on a format-only batch of 49 `{ている}`→`ている` fixes produced 5 flags, **5/5 rejected** — all pre-existing partial-reading false positives (`果|は` in 果てる, `跳|は`/`上|あ`) or a hallucinated kanji (`捩`→ね on an entry that actually reads `捻|ねじ`). Consistent with the standing pattern that the screening model over-flags correct partial okurigana readings despite the prompt's explicit instruction not to. For format-only systemic-fix batches the self-check is reassuring (it confirmed the edit introduced nothing) but its furigana flags on untouched pairs are near-zero precision; the decision ledger now carries the per-flag data (`reviews/decisions.jsonl`, src=furigana) for the metrics-trend precision table.
