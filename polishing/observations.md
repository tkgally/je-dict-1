...
- [pattern] new-entries 2026-06-20: oldest unprocessed candidates (Feb-2026 vintage, e.g. C06045 権使, C08195 些道, C09223 個尊, C10277 怒燥, C09527 多角的一面) are largely typos or non-words — the standard "oldest first" fallback lane is mostly junk. The 14 "seen in entry" candidates were all high quality. Suggest a candidate-list cleanup pass (clean_up_candidates_list.md) to purge the pre-March junk so the fallback lane is usable.
- [tooling] new-entries 2026-06-20 self-check: review_accuracy.py (gemini-2.5-flash) flagged 炭水 formality "informal" as error and suggested "colloquial" — but "colloquial" is a domain value, not a valid formality (schema: formal/neutral/informal/vulgar). The reviewer prompt's tags-dimension does not know the formality enum; consider embedding the valid formality values like VALID_SEMANTIC is embedded, to suppress this false positive.

## 2026-06-20 — routine(polish) session (frontier 06210–06213, priority lane scan)
- [pattern] Frontier entries created 2026-01-17 and last touched 2026-04-10 by claude-opus-4-5 (06210 onward: すりおろす, 誘い込む, 殴り倒す, and the 06213+ proverb/idiom block) have NO inline word links in examples OR notes — they predate the inline-link polishing step. The whole 06200–06250 band (compound verbs + four-character idioms/proverbs) likely needs an inline-link sweep. Furigana and structure are otherwise clean.
- [tooling] The notes-quality priority list (polishing/priority/notes.txt) ranks heavily-polished basic/core-tier structured-note entries (へ, 小さい, 遅い, 何でも) at the very top, but all four were already complete (full inline links, good cross-refs, conjugation tables) and needed zero changes. The scorer appears to penalize the structured/sectioned note style these grammar/adjective entries use. Consider tuning score_note_quality to recognize sectioned notes and the auxiliary structured fields (predicates_requiring, particle_contrasts, fixed_patterns) so the priority lane stops surfacing finished entries.
- [entry] 00304 nandemo: sense 3 ("by all means / at any cost", examples ex11–15) rests on 何でも alone carrying an adverbial "at any cost" meaning, which is dubious standard usage — natural phrasings are 何が何でも or どうしても. Worth a deeper review/possible sense removal; out of scope for a link-only polish pass.

## 2026-06-21 — routine(accuracy-review) session (furigana+tags, 7815–8037)
- [tooling] **Systemic invalid-semantic-tag drift across the ~7815–8037 creation block.** A deterministic scan against `build/validate_tags.VALID_SEMANTIC` found **163 of 223 entries (73%)** in this range carrying out-of-taxonomy semantic tags. This run migrated the 43 that the cross-model accuracy reviewer flagged at `error` severity (e.g. architecture/house→building, social→communication, speech→communication, economy→economics, progress→change, 'change of state'→change). The **remaining 120 entries** still have invalid tags. The drift families are large and mostly 1:1-mappable: free-form domain words (`career`, `lifestyle`, `place`, `document`, `accommodation`, `commerce`, `accounting`, `employment`, `logistics`, `personnel`), underscore/space variants (`daily_life`, `daily life`→`daily-life`; `Japanese_cuisine`/`Japanese cuisine`→drop, `food` already present), and body/health splits (`body`→`body-part`, `sleep`→`health`, `injury`→`health`). This is **systemic-fix territory**, not per-run accuracy-review: recommend expanding `build/check_tag_drift.py`'s `TAG_MIGRATION` map to cover these families, committing it, then running a deterministic+spot-checked sweep over the whole block (and likely the adjacent 7000–8500 creation cohort, which appears to share the free-form-tagging origin). The cross-model reviewer's `tags` dimension corroborated the same problem but only surfaces a fraction per pass and at high adjudication cost; the deterministic detector is the scalable instrument here.
- [pattern] The furigana screening pass over this already-structured range ran at ~0% precision: all 33 screening flags were false positives (truncated-reading rendering artifact in the screener prompt, rendaku in compounds like かすれ声/覚え書き/目覚まし時計, okurigana splits, and deliberate dual-reading documentation like 粗利 あらり/そり). Consistent with the 0–5% precision measured 2026-06-10/11 on polished ranges; the known-noise shortcut (bulk-reject, skip deep pass) applied cleanly.

- [pattern] 2026-06-21 (routine_002): notes-priority ranking is stale — the top entries in polishing/priority/notes.txt are already-polished basic-tier words (he, chiisai, osoi, nandemo, tonari, ude) that score low only on note length/structure heuristics. 6/6 reviewed needed no changes. Consider a recency/coverage guard in prioritize_polishing so heavily-polished basic entries don't dominate the notes lane.
- [pattern] 2026-06-21 (routine_002): the proverb/yoji-jukugo block at 06214+ had no inline-link coverage despite recent (2026-06-16) modified timestamps — those bumps were not comprehensive polish. Frontier just reached them; linking added. Watch for more unlinked idioms above the frontier.
