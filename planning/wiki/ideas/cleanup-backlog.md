...
**Last updated**: 2026-07-18 (wiki harvest of the 3 loose 2026-07-17 observations from a routine polish run + a 2026-07-17 accuracy-review — all reinforcements of established patterns: **P21** — the sequential frontier begins **draining** the 06500 Jan-2026 creation block, 06517–06520 ガードレール/足場/応急処置/じゃんけん receiving full `⟦...⟧` coverage in examples AND notes (first entries of the flagged 06500 block to be linked), ~15 candidates harvested from their `noentry` glossaries — the band is now drained sequentially from the cursor, though the dedicated 06500–06999 sweep remains higher-throughput; **P20** — the off-vocab creation-batch cohort extends into the **15567–15766** band (a 2026-07-17 accuracy-review flagged 56/200 = 28% on `tags`, ~45 genuine off-vocab migrations vs ~15 rejected in-list narrowness nits). Tooling-side: **item 17** twenty-first confirmation (same 28% two-regime split), **item 24** 100% FP on the 15567–15766 furigana screen (19/191, all documented families), **item 20** fifty-fourth no-op confirmation + a precise per-POS restatement of scorer-bug #2 (the `adjective-i` `required_sections=['usage']` misses an unheadered opening usage paragraph). Activity H metrics trend not due (5 new lines, 286→291 runs). Prior 2026-07-15 (wiki harvest of the 9 loose 2026-07-15 observations from two routine polish runs + a 2026-07-15 accuracy-review — all reinforcements of established patterns: **P20** — the off-vocab creation-batch cohort reaches the **15100s** [a 15100–15153 accuracy-review migrated ~30/54 off-vocab tags in-run], recommends a `check_tag_drift --check unknown-semantic` sweep over 15000–15500; **P21** — the zero-inline-link band reaches **06490–06499** with the tractability datum that example-linking is uniformly cheap while notes-linking cost tracks note density [dense appliance/composition glossaries defer, ordinary notes complete in-run]; **P17** — a fifth formality sub-family, everyday **colloquial i-adjectives** auto-tagged `formal` [06498 しつこい `formal`→`neutral`], with a cheap `adjective-i`+`formal` detector cut. Tooling-side: **item 17** twentieth confirmation [the `general`-too-broad family now surfaces on the create-era frontier §4 self-check too, 3/5 rejected], **item 20** fifty-first/fifty-second no-op confirmation, and **new item 33** [`prioritize_polishing.py --task notes` crashes `KeyError: 'furigana'`]. Prior 2026-07-14 (wiki harvest of the 20 loose 2026-07-13/14 observations from four routine polish runs + a systemic-fix run + accuracy-review sweeps — all reinforcements of established patterns plus one genuinely-new cluster: **P11 (new)** — the ～ハラ/harassment noun cluster is internally inconsistent across four different (mostly wrong) tag treatments [08788 マタハラ off-vocab `social-issues`, 06466 モラハラ wrong-category `emotion`, 05904 パワハラ / 05905 セクハラ both placeholder `general`] → a targeted harassment-cluster `tags` pass settling on `society`; **P21** — the zero-inline-link create-era band runs unbroken through **06465–06479** across four runs [06465–06470 slang/loanword coinages チルい/モラハラ/ガチ勢/KY, 06471–06474 堆肥/果樹園/アンチ/プロフィール, 06475–06479 ユーチューバー/画面共有/プッシュ通知/入会金/添加物], the whole 06400–06500 confirmed a contiguous unlinked zone; **P20** — the free-form/off-vocab tag cohort confirmed contiguous into the **14000s** [a 2026-07-13 systemic-fix run migrated 19 legacy off-vocab tags over 14387–14899], dict-wide residue ~**6,235 entries** → the deterministic `check_tag_drift --check unknown-semantic` sweep still beats per-range review; **P17** — a fourth formality sub-family, everyday **katakana loanwords** auto-tagged `informal`/`formal` against their own neutral register [06474 プロフィール `informal`→neutral, 06475 `formal` vs its "REGISTER: Neutral" note]; **P13** — the placeholder sole-`general` + occasional plain-wrong tag [入会金→`education` should be `money`] reaches the 06475–06479 early-2026 tech/loanword cohort, cross-model flags them at `error` severity → a ~06400–07000 tags-dimension sweep pairs with the P21 link sweep; **P9** — the cosmetic-furigana-wrapper backlog thins sharply above ID ~11000 [~8 flags/500-ID block vs 18–33 in 06000–10999], remaining bulk in the 23000–29999 new-entry blocks (36–110 each) + the pre-06000 backlog → reprioritize wrapper sweeps by density. Tooling **item 20** [forty-seventh/forty-eighth no-op confirmation + the consolidated scorer-bug root-cause], **item 17** [seventeenth/eighteenth confirmation, 14187–14386 & 14387–14899 ~17–24% flag rate, mostly in-list-narrowness rejects], **item 21/31** [review_runner 2-min wall-clock ceiling on 20–30-entry batches], **item 23** [C06000–C14000 corpus block confirmed heavily polluted], **item 6** [14387–14899 migration families]. Prior 2026-07-12 (wiki harvest of the 8 loose 2026-07-11/12 observations — reinforcements plus one genuinely-new tooling item: **P21** — the zero-inline-link create-era band extends unbroken through **06451–06456** across two routine polish runs [06451 自動運転 / 06452 蛇の目傘 had zero `⟦...⟧` in examples AND dense notes at birth, hand-linked ~22/~19 links; 06453–06456 hand-linked at the frontier, comprehensive cursor → next:06457]; band now unbroken ~06150→06456; paired candidate harvest 実用化/移動手段/センサー [C22295–C22297] + 唐傘/写真撮影 [C22298–C22299]; **throughput datum** — fully linking a dense-note 06450s entry is ~15–25 lookups, so realistic frontier throughput on unlinked dense-note ranges is ~4–5 entries/run vs 20–25 on already-partially-linked ranges [budgeting note for future frontier runs]. See also Tooling **new item 32** [`add_adjective_conjugations.py` freezes the first variant of slash-variant i-adjective headwords like `{速|はや}い／{早|はや}い`; both existing cases 00475/00514 were buggy and hand-fixed, live scope now zero — a regression-guard item], **item 19** [three more stale `：noentry` markers in already-polished entries — 06484 噛み殺す→29106, 06731 御影石→29109, 07006 ビハインド→29128, "likely hundreds dict-wide"], **item 17** [sixteenth confirmation — a 13725–13874 accuracy-review flagged 37/150 ≈25%, dominated by 21 rejected `general`-too-broad in-list nits + formality FPs on entries whose own notes support `formal`, only 4 applicable not-in-list migrations], and **item 20** [forty-fifth/forty-sixth no-op confirmation — the only real fixes were structural/off the notes-scorer's axis: 速い's frozen conjugation table + stale `：noentry` links]. Prior 2026-07-11 (wiki harvest of the 9 loose 2026-07-10/11 observations — all reinforcements of already-filed patterns: **P20** — off-list semantic tags cluster densely in the **13650–13724** range [a 2026-07-11 accuracy-review flagged 20/75 entries carrying tags outside `VALID_SEMANTIC`: body, transport, thought, character, quality, household, symbols, stationery, government, document, space, shape, academic, perception, relationship — migrated in-run]; the whole **13650–14149** block likely predates closed-vocabulary enforcement, so it is the next contiguous `check_tag_drift.py --check unknown-semantic` systemic-fix target above the already-mapped 13xxx cohort. **P21** — the zero-inline-link create-era band extends through **06441–06447** across two 2026-07-10 routine polish runs [06441–06444 陰キャ/陽キャ/アナログ/投げ銭 + 06445–06447 不祝儀/クーポン/延長, all zero `⟦...⟧` in examples AND dense notes at birth, Jan-2026 create-era, ~15–40 links/entry once hand-linked; both runs hand-linked their frontier entries as the comprehensive cursor advanced]; band now unbroken ~06150→06447; paired candidate harvest ネットショッピング/大道芸人/ギターケース/スーパーチャット/スパチャ [C22281–C22285] + 関係性/薄墨/御仏前/ホームチーム [C22286–C22289]. See also Tooling **item 17** [a 13550–13649 accuracy-review ran 26/99 = 26% flagged, dominant noise family `general`-too-broad (n=12, all rejected per §A), real applies the two not-in-list tags legal→law 13640 / body→body-part 13649 + one clear mismatch existence→health on 病む 13611], **item 20** [priority-lane forty-third/forty-fourth no-op confirmation — 5/6 and 4/4 no-op on the same closed-tier basic/function set; regeneration proven futile a third time same day, cursor advanced past examined no-ops instead], **item 21/31** [`review_runner.py` furigana screening ~8 s/entry with intermittent multi-minute stalls in the Routine env, scoped to a 27-entry sub-range this run], and **item 23** [candidate_words.json heavily diluted with compositional/derivable junk — a 2026-07-11 new-entries run surveyed ~600 candidates to hand-pick ~12 genuine standalone lexemes]. The 06444 nagesen をする/する duplicate artifact was cleaned in-run [no follow-up].) Prior 2026-07-10 (wiki harvest of the 6 loose 2026-07-09/10 observations — all reinforcements of already-filed patterns: **P20** — two accuracy-review runs measured the **13300–13549** block at a uniform ~40%/23% off-vocab tag density [13300–13349 20/50, 13350–13549 46/200], all 1:1-mappable to in-list tags [motion→movement, nature-*→nature, house→building, season→time-season, food-cooking→food, *-concept/*-speech families]; both runs conclude the whole 13xxx block is a single mostly-1:1-mappable off-vocab creation batch a dedicated `check_tag_drift --check unknown-semantic` systemic-fix sweep would clear far faster than per-range review; cohort now contiguous through 13549. **P21** — the zero-inline-link create-era band reaches **06438–06440** [手の甲/経理/タイムライン, zero `⟦...⟧` in examples AND dense notes, ~20–40 links/entry yield once hand-linked]; paired candidate harvest ひび割れ/足の甲/仕訳/マネージャー/フィード/もごもご [C22275–C22280]; band now unbroken ~06150→06440. **P13** — the frontier-applied high-precision sole-`general`→specific family persists past 06430 [06438 手の甲→body-part, 06439 経理→business, both applied in-run], distinct from the accuracy-review lane's rejected too-broad narrowness noise. See also Tooling **item 17** [13350–13549 flag mix: not-in-list flags high-precision/applied, `general`-too-broad the rejected residue, net <20% noise] and **item 20** [priority-lane no-op reinforcement + stale-`noentry` markers 05766→29117 / 05775 as an item-19 detector signal].) Prior 2026-07-08 (wiki harvest of 2026-07-06/07/08 loose observations: **P21** — the zero-inline-link create-era band is **now confirmed unbroken ~06150 → 06430**, the sequential frontier having climbed through it [06418–06422 保活/泡立てる/ハッシュタグ/インフルエンサー/飼育 social/parenting + cooking + katakana social-media vocab; ~6423–6426 academic + construction/building nouns; 06427–06430 断熱/防水/灌漑/害虫 technical noun/suru-verb], all furigana-complete but zero `⟦...⟧` in examples *and* notes until each run hand-linked its frontier entries (comprehensive cursor now next:6431); the datum this window is that link **yield is high** here — the dense collocation/composition note blocks are full of domain compounds that now DO have entries (排水/撥水/防塵/益虫/殺虫剤…), so the ~06400+ band stays the highest-value contiguous inline-link target. **P20** — the out-of-taxonomy tag cohort reaches the **12507–13199 band**, a dense off-list pocket from single creation batches [12520–12577 systematic off-vocab compounds action-construction/action-creation/thought-cognition/society-role/culture-traditional-arts/language-writing/place-general/…; 12674–13199 65/526 entries genuinely off-list: place/war/degree/quality/material/crime/information/sensation]; two 2026-07-06/08 accuracy-review runs migrated the provably-safe hyponym→parent tags [39 in 12507–12673, 28 in 12674–13199] and escalated **37** judgment-dependent tags to `needs_curator.txt` — recommends a dedicated systemic-fix pass with an **expanded migration map** [degree→quantity/general, place→geography/building, quality→evaluation, information→cognition/communication] over this band and its adjacent same-signature batches. **P13** — the placeholder sole-`general` create-era signature continues onto concrete nouns at the **06420/06427–06430 frontier** [06420→technology, 06430 害虫→animal-insect — both applied at the frontier as clearly-correct single-domain]; recommends a `check_tag_drift` sweep over 06427–06600 to surface more sole-`general` placeholders ahead of the crawl). Prior 2026-07-06 (wiki harvest of 2026-07-05 loose observations: **P21** — the zero-inline-link create-era band **reaches the 06405–06417 early-2026 loanword/business cohort** [06405–06410 ノマド/コワーキング/イノベーション/ベンチャー/ソリューション/脱炭素 + 06411–06417 further recent loanword/business vocab, zero `⟦...⟧` in examples *and* notes, several carrying sole-`general` semantic tags; band now unbroken ~06150→06417]; **P13 + systemic-fix recommendation** — the 06405–06410 katakana business/tech entries pair the naked-link gap with the placeholder sole-`general` tag signature, so a **targeted link-completeness + sole-`general` retag sweep over the early-2026 katakana business/tech ID band (~06400+) would be higher-yield than the sequential frontier crawl**, which advances only a handful of these entries per run). Prior 2026-07-04 (second wiki harvest, of 2026-07-04 loose observations: **P21** — the zero-inline-link create-era band **crosses into the 6400s modern-vocab/loanword cluster** [06398–06401 マインドセット/ワークショップ/仮想通貨/ブロックチェーン + 06402–06404 電子決済/QRコード/ロボット, a Jan-2026 batch, zero `⟦...⟧` in examples *and* notes; 06402–06404 hand-linked at the frontier as the comprehensive cursor advanced to next:6405; band now unbroken ~06150→06404, character shifting from idioms to loanword/tech vocab]; **new Priority 23** — a 20-entry block **29181–29200 (a 2026-06-12 batch) missing `metadata.vocabulary_tier`** entirely [schema-optional so CI passed; should all be `general`; bounded mechanical-safe systemic-fix, added to `backlog-queue.json`]). Prior 2026-07-04 (wiki harvest of 2026-07-03 loose observations: **P21** — the zero-inline-link create-era band reaches the **06389–06393** four-character-idiom cluster [壊滅的/抱腹絶倒/傍若無人/意気投合/厚顔無恥, all furigana-complete but zero `⟦...⟧` links, some despite recent `modified` dates — 06392 was 2026-06-25; band now unbroken ~06150→06393, inline-link completion still lagging other edits across 06000+]). Prior 2026-07-03 (wiki harvest of 2026-07-02/03 loose observations: **P21** — the zero-inline-link create-era band reaches **06376–06388** [three consecutive frontier runs — sessions 004/007/008 over 06370–06388 — all fully naked in examples *and* notes; band now unbroken ~06150→06388; the 06300–06400+ range confirmed a contiguous un-linked zone, dedicated ~06300–06500 sweep restated]). Prior 2026-07-02 (wiki harvest of 2026-07-01/02 loose observations: **P11** — in-list-but-wrong-category tag drift persists into the 11500s general-tier cohort [11573–11646 accuracy-review, 5/14 genuine ≈36%: abstract/loanword nouns on oddly-specific in-list domains — トレード/作画→`leisure`, 入浴→`consumption`; invisible to P20/`concrete-noun-domain-mismatch`; recommends a targeted katakana/suru-noun `tags` sweep]. Prior 2026-07-01 (wiki harvest of 2026-07-01 loose observations: **P20** — the off-vocab tag cohort reaches the **11515–11553 band** [a granular hyphenated tag set — `action-*`/`religion-*`/`quality-*`/`person-occupation`/… — from a ~Feb-2026 claude-opus-4-6 batch; 26/72 entries, migrated 1:1; recommend a 11500–12500 `--check-no-new-unknown` sweep]; **P11** — the wrong-category drift reaches **function words** [06355 どうせ, an adverb, tagged `furniture` + a contradictory `formal`; cleaner deterministic signal than the noun cases — a physical-object tag on an adverb/particle POS — filed as a new `check_tag_drift` check via Tooling item 6]). Prior 2026-06-30 (wiki harvest of 2026-06-29/30 runs: **P20** — the off-vocab tag cohort reaches the **11300s 不-/中-/下-/両- compound band** [1:1-mappable families quality→descriptive, position-direction→direction, people→person, place→geography; 2026-06-30 accuracy-review migrated 24/35 — recommend folding into `check_tag_drift.py`'s map for a deterministic sweep]; **P21** — the zero-inline-link create-era band reaches the **06338–06343** four-char-idiom/compound-verb cohort [band now unbroken ~06150→06343; gap concentrated in this mid-ID 2026-01-17 batch — recommend a ~06338–06500 sweep]. Prior 2026-06-28 (second wiki harvest, of 2026-06-28 runs): **P20** — the free-form tag cohort reaches the **10700–11000 katakana/slang/loanword band** [10716–10887 45/67 flags off-vocab; a dense 10905–10934 pocket from a 2026-02 claude-opus-4-6 batch; cohort now contiguous through ~10947 — recommend one scoped `check_tag_drift` sweep over 10700–11000]; **P13** — the placeholder-`general`/`science` signature continues at the **06309–06340 frontier** [astronomy/stepladder concrete nouns, curator-bulk-migration territory]. Prior harvest of 2026-06-27 routine runs: **P20** — the free-form tag cohort reaches the **10400–10700 band** [~50/100 invalid in 10450–10549; reviewer migrated 64 but *missed* ~9 in 10550–10715 → deterministic sweep still needed in nominally-reviewed ranges]; **P21** — zero-link create-era band reaches the heavy **06294–06308** cultural/medical/finance cohort [15–50 lookups/entry, much `noentry`]; **P17** — third sub-family, **verb-suru** entries carrying a template-default `formal` [06307 仲直りする contradicts its own notes]; **P13** — the under-specified `general`/`work` on the 06298–06303 medical/finance batch is itself a create-era batch signature [4/5 applied as clearly-correct single-domain]. Prior 2026-06-26 (harvest): P20 updates — daily-life/errands cohort continues into 9741–9814 [20% invalid] + a new shopping/tech cohort at 9815–9849 [60% invalid, migrated]; **enforce-side shipped** [off-vocab ratchet `--check-no-new-unknown` + baseline, now a CI step — gates new drift, legacy tail still gradual]. P21 update — the zero-inline-link band reaches the 06271–06281 mimetic/idiom/拝〜-keigo cohort [recommend a ~06275–06600 inline-link sweep]. Prior 2026-06-25: P20 update — denser daily-life/errands sub-batch at ~9657–9740 [48/84 = 57% invalid tags, migrated; recommend a confirming 9600–9800 `check_tag_drift` sweep]. Prior 2026-06-24: P20 free-form cohort into 9240–9456 [56%, ~86 residual]; **new Priority 22** — inconsistent free-text `part_of_speech` display field; 2026-06-23: P21 band through 06246; P20 8,698 dict-wide flags, 8633–9239 long tail has no 1:1 map; P9 06231 kana-inside-group + nested braces)
...
**Sibling defect found 2026-07-25 — wrong verb *class* on a genuine verb (the guard does not cover this)**: the resolved sweep above removed conjugation tables from entries that are **not verbs**. A 2026-07-25 routine polish run found the adjacent, still-open failure: a **genuine verb tagged with the wrong class**, whose entire conjugation table is therefore fabricated morphology. **06624 甘える** (ichidan) was tagged `verb-godan` / `godan-ru`, so `add_conjugations.py` generated 甘えらない / 甘えります / 甘えった — plausible-looking forms that do not exist. The `add_conjugations.py` exact-enum POS guard cannot catch this: the POS *is* a verb POS, just the wrong one, so every downstream form is silently wrong for learners. A scan of all `verb-godan` entries whose reading ends in **-eru** returned **38 hits, 36 of them genuine godan** (the known 帰る / 滑る / 蹴る / 返る false-positive family) and **one further real case, 09361 バックレる**, contradicted by its own example (バックレた, an ichidan past). Both were fixed in-run with `add_conjugations.py --force`.

The high-precision discriminator the run proposes — and the reason this is a *detector* item rather than a rename sweep — is to **cross-check the entry's own examples against its declared class**: flag a `verb-godan` entry whose examples contain the **ichidan** past/て-form of its headword (甘えた/甘えて for a headword whose godan past would be 甘えった). That inverts the -eru heuristic's 5% precision into something close to certainty, because the example sentences were written by a different process than the conjugation generator. Filed as [Tooling item 34](tooling-backlog.md#34-verb-class-mis-assignment-detector-fabricated-conjugation-tables-on-genuine-verbs); scope dictionary-wide is unmeasured (2 confirmed, both fixed).
...
**Update 2026-07-24 (cosmetic sweep progress)**: The routine `systemic-fix` mode drains the cosmetic sub-patterns (o/go-prefix-inside-wrapper, pure-kana wrappers, over-wrapped okurigana/compound) incrementally, one ID slice per run, with per-entry verification. As of 2026-07-24 the 20000–20999 slice is done (24 instances / 18 entries; 1056→1032), leaving **~1032 filtered instances** across these three sub-patterns; the running per-slice log lives in `planning/wiki/ideas/backlog-queue.json` → `furigana-cosmetic-wrappers`. Next ready slice: 21000+ and the larger pre-06000 backlog.
...
**Update 2026-07-25 (second — the cohort continues past 17760 into a themed 18250–18300 pocket; detector 6,721→6,696)**: A 2026-07-25 accuracy-review over **17911–18345** found the off-vocab creation-batch signature still running, now concentrated in a themed pocket at **18250–18300** (`security`, `vision`, `speech`, `group`, `tourism`, `place`, `posture`, `strategy`, `event`, `shape`, `body`, `biology`, `reading`) — migrated in-run. The observing run's judgment is worth recording as a **prioritisation datum for the whole priority**: the `tags` dimension of `review_accuracy.py` "reliably surfaces them and the migrations are unambiguous, so this is the cheapest remaining lever on the [5,448-entry] off-vocab backlog." That is the same conclusion the P20 chain has reached from the systemic-fix side (a dedicated `check_tag_drift.py --check unknown-semantic` sweep), now stated from the review side: **both lanes are high-precision on this cohort; the binding constraint is throughput, not judgment.** The dict-wide `unknown-semantic` residue stands at **6,696 flags across ~10,037 entries** (detector 2026-07-25, down from 6,721 earlier the same day; semantic-mismatch 813, sole-general 3,823 alongside). Same `unknown-semantic-tags` backlog item; reinforces [Tooling item 27](tooling-backlog.md).
...
**Update 2026-07-25 (the band is confirmed to continue *above* the cursor — 06627+ spot-checked zero-linked — and the frontier lane's per-entry cost is now quantified at 10–15 min)**: A 2026-07-25 routine polish run reported that **entries from ~06627 onward have zero inline-link coverage** (0 `⟦…⟧` markers in examples *and* notes; 06627–06632 spot-checked, all empty). Two things make this update more than the routine "band extends by five" datum:

1. **It is a forward-looking check, not a rear-view one.** Previous P21 updates recorded the band as far as the frontier had *drained* it; this run sampled *ahead* of the cursor and found the same emptiness, so the band is not merely "unbroken ~06150 → 06626" but is expected to continue for the whole create-era block above it. Every entry above the cursor needs full tier-1 link work.
2. **The frontier lane's economics are now measured**: roughly **10–15 minutes per entry** for a 9-example entry. At ~5 entries/run that is the dominant cost of the polish mode from here on — which is the quantitative form of the standing recommendation that a **dedicated 06500–06999 (and now 06600–07000) notes-and-examples linking sweep** would beat the ID-sequential crawl, and the strongest argument yet for unblocking [Tooling item 15](tooling-backlog.md) (the unlinked-label lint) so the sweep can be run as a detector-driven systemic-fix batch rather than by hand.

Related sub-pattern filed the same day as its own priority: inline links whose *base form* carries furigana braces ([Priority 24](#priority-24-inline-link-base-forms-written-with-furigana-braces)) — a mechanical defect introduced *by* this linking work, so a base-form sweep should run alongside (or just after) any bulk link backfill.

## Priority 24: Inline-link base forms written with furigana braces

**Source**: Routine v2 polish run, 2026-07-25 (found and fixed live in 00969_mata)

The inline-link syntax is `⟦surface→base：entry_id⟧`, where **`base` is plain kanji**
(`⟦来た→来る：00254_kuru⟧`). **44 entries** instead write the base form with furigana
wrappers — `→{来|く}る：00254_kuru` — putting reading braces in a field that is never
rendered as ruby. The links still resolve (the entry_id is what matters), so this is
cosmetic-to-structural rather than a dead-link class, but it makes the base form
inconsistent with the documented format in the
[`inline-word-links` skill](../../../.claude/skills/) and with the other ~thousands of
links, and it will confuse any future tooling that parses base forms (e.g.
[Tooling item 11](tooling-backlog.md#11-inline-link-target-id-resolution-gate-in-validatepy-or-pre-commitci)'s
proposed base-form-vs-target consistency check, which would have to strip wrappers first).

**Why it is a good systemic-fix candidate**: the transform is a regex **restricted to the
segment between `→` and `：`** — strip `{`/`}` and the `|reading` half inside that segment
only — which provably cannot touch surface text, readings elsewhere in the sentence, or
the target id. It is one of the few genuinely `mechanical-safe` items on this page (still
validated + spot-checked before commit per §B). Detection is a one-line grep for `→[^：⟧]*{`.

**Scope**: 44 entries as measured 2026-07-25 (one, 00969_mata, fixed in-run → ~43 remaining).
Indexed in `backlog-queue.json` as `inline-link-braced-baseform`.

## Priority 25: Auxiliary verbs written in kanji after the te-form

**Source**: Routine v2 polish run, 2026-07-25 (01092_oku)

Contemporary Japanese writes **auxiliary** (non-lexical) verbs in hiragana: 〜ておく,
〜てみる, 〜てくる, 〜ていく, 〜てしまう. The polish run found **01092_oku's own examples**
writing the auxiliary with kanji — 〜て{置|お}く in **5 of its examples** — while the same
entry's notes correctly wrote it in kana. That is a self-inconsistency inside a *basic-tier*
entry teaching exactly that auxiliary, and it models an orthography learners should not
copy.

**Suggested detector**: flag a te-form (`て`/`で`) immediately followed by a kanji-written
member of the auxiliary set — 置く / 見る / 来る / 行く / 仕舞う（しまう）/ 貰う / 呉れる /
遣る — inside `examples`. The detector must be a review queue, not an auto-rewriter: the
same kanji spellings are correct when the verb is **lexical** (本を{置|お}く "put the book
down", 学校に{行|い}く "go to school"), so the discriminator is "immediately preceded by a
te-form of another verb", and even that needs per-entry confirmation (て can end a clause
that is followed by an independent lexical verb).

**Scope**: unmeasured dictionary-wide; one confirmed entry (01092, fixed in-run). Worth
scoping with the detector before deciding whether it merits a sweep — the te-form + kanji
auxiliary bigram is cheap to count.
