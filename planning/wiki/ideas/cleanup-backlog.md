...
> Resolved, retired and refuted items are in [cleanup-backlog-resolved.md](cleanup-backlog-resolved.md) (moved 2026-09-26).
...
proposes the same instrument via [Tooling 6](tooling-backlog-resolved.md#6-tag-drift-detector) — so it was
...
An inline link `⟦surface→base：entry_id⟧` whose `entry_id` names no existing entry renders as a dead cross-reference on the live site. `build/validate.py` does not check target resolution, so these pass every gate the project has — see [Tooling 11](tooling-backlog-resolved.md#11-inline-link-target-id-resolution-gate-in-validatepy-or-pre-commitci) for the full measurement, the detect snippet, and the gate that has to ship first.
...
[Tooling 44](tooling-backlog-resolved.md#44-consistency-check-non-neutral-formality-with-no-register-statement-in-the-notes).
...
[Tooling 55](tooling-backlog-resolved.md#55-detector-contrast-words-named-in-notes-prose-but-absent-from-cross_references).
