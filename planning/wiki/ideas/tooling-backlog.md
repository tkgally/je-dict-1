...
> [Cleanup P35](cleanup-backlog-resolved.md#update-2026-08-02--provenance-split-12-were-wrong-when-written-not-stale-and-that-subclass-is-closed).
...
[Cleanup P24](cleanup-backlog-resolved.md#priority-24-inline-link-base-forms-written-with-furigana-braces) —
...
[Cleanup P41](cleanup-backlog-resolved.md#priority-41-conjugation-tables-generate-the-potential-of-a-verb-that-is-already-potential).
...
[Cleanup P43](cleanup-backlog-resolved.md#priority-43-the-0680007100-block-is-96-unlinked--a-bounded-batch-not-a-frontier-problem)'s
...
### 136. `kanji_list.json` carries a second, stale count field

The 2026-08-22 accuracy-review run reported that `kanji/kanji_list.json` holds two count fields
in its metadata — `total_kanji`, which `build/update_kanji_index.py` keeps current, and a legacy
`count`, which nothing updates — and that the gap widens every time a run assigns new kanji IDs.

Verified in one read, and the numbers are exactly as reported: **`total_kanji: 2798`,
`count: 2794`**, against an actual **2798** kanji records in the file. So `total_kanji` is right,
`count` has been wrong since at least four kanji ago, and any consumer that picked the shorter
name is reading a number that only ever drifts further out.

Small and unambiguous: either have `update_kanji_index.py` write both fields, or drop `count`
from the file. Dropping it is the better fix — two names for one quantity is how the drift
started — but it needs a grep for readers first. **Scope**: one field, one script.
