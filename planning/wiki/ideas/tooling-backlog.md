...
**Update 2026-07-25 (a second, sharper case — and a cheap way to detect the family with an existing tool)**: A 2026-07-25 routine polish run found the mirror image of the 00565/00760 case: **06626 見栄 (みえ)** carried a **sense 2 "kabuki dramatic pose" whose three examples were all written 見栄を切る**, while the correct entry **29012 見得** already existed and *is* that sense. So the defect is not only "a sense duplicates another entry" but "a sense on entry A is written with entry A's kanji when the sense belongs to entry B's kanji" — a **same-reading kanji pair split across a correct entry and a wrong sense on its homophone**, which teaches the wrong orthography rather than merely duplicating content. The run removed sense 2 from 06626, cross-linked the two entries, and documented the 見栄を張る / 見得を切る verb split.

The tooling datum: **`build/find_merge_candidates.py` already groups entries by reading**, so pairs like this are probably *already visible* in its output and just need the right filter — a **targeted pass over same-reading pairs where one entry has a sense the other entry `is`** (heuristic: sense gloss of A overlaps the headword gloss of same-reading B, and A's examples for that sense use A's kanji). That makes the detection side of this item substantially cheaper than the original spec's per-sense cross-reference walk: run the existing grouping, then rank homophone pairs by gloss overlap. The **collocation split is the confirming signal** (見栄を張る vs 見得を切る): when two same-reading entries take different verbs, a sense on one that uses the *other's* collocation is a near-certain misplacement.
...
**Update 2026-07-25 (the strongest datum yet — nine dead links written in a single run, all validating clean, and the documented check is itself misleading)**: A 2026-07-25 routine polish run wrote **nine inline links with plausible-but-wrong entry IDs** (`00478_tomodachi`, `09048_koukyuu`, `09502_noni`, …) while doing ordinary frontier link work. **All nine passed `build/validate.py` cleanly**; they were caught only by an ad-hoc scan of the link targets against the set of on-disk entry filenames. Two things escalate this item:

1. **The gap is actively mis-documented.** Both `CLAUDE.md` and `prompts/comprehensive_polish.md` advise `validate.py --id X | grep -i "word link"` as *the* link check — so a polishing session that follows the documented procedure believes its links were verified when nothing checked them. Whatever "word link" output validate.py does emit is not a target-existence check. Until the gate ships, the prompts' wording should be corrected (or the ad-hoc scan promoted into them), because the current text converts a missing check into a *false assurance*.
2. **The error is generated by the very work the Routine now spends most of its polish budget on.** With [Cleanup P21](../ideas/cleanup-backlog.md#priority-21-unlinked-自動詞他動詞-labels-and-particles-in-compound-verb-notes) putting the frontier lane at 10–15 min/entry of hand-written linking, link volume per run is at an all-time high; a per-run dead-link rate of nine is a steady inflow of exactly the defect this gate prevents. Ship order suggestion: the **standalone `build/check_inline_links.py`** (cheap, no schema surgery) wired into `make validate` + CI, then a one-time full-corpus scan to size the existing damage.

This is the same class the §4 self-check cannot cover — an independent *model* review does not resolve IDs against the filesystem — so it must be deterministic tooling.
...
**Update 2026-07-25 (twelfth confirmation — and the first side-by-side throughput measurement against `review_accuracy.py`: ~8/min vs ~29/min)**: A 2026-07-25 routine accuracy-review ran both reviewers over the same **17911–18345 (~490 IDs)** range and measured them against each other: **`review_runner.py --pass screening` ran at ~8 entries/min and was cut off at 18345 (421 of 490 covered)**, while **`review_accuracy.py` completed the full range at ~29 entries/min**. That is the cleanest statement yet of the constraint — the furigana screener is **~3.6× slower per entry than the accuracy reviewer against the same range and the same provider**, so the two passes cannot be sized alike, and a range chosen to fit the accuracy pass will always truncate the screening pass. The observing run's prescription: **add concurrency to the screener, or size screening ranges to ~350 entries per Routine run** (its wrapper budget); the standing ≤~100–200 ID guidance from shorter-wrapper runs remains the conservative floor, and the exact cap tracks the wall budget a given run receives. Twelfth truncation-class observation. Combined with [item 24](#24-non-hiragana-reading-lint-cheap-replacement-for-the-furigana-screeners-true-positive-class)'s repeated ~0–2% precision measurements, the throughput gap sharpens the strategic question this item has been circling: **the slowest reviewer is also the least precise one**, so concurrency work on the screener should be weighed against simply reserving it for never-reviewed ranges and letting the deterministic lints carry the rest.
...
**Update 2026-07-25 (reinforcement — ~200 candidates scanned to curate 15 — plus a genuinely-new sub-fix: the "seen in entry" logger must dedupe by *reading*, not just surface)**: A 2026-07-25 routine new-entries run reconfirmed the <10%-signal finding on the oldest candidates with the standard families (OCR/coinage non-words 権使 / 些道 / 個尊 / 怒燥 / 発炭, compositional number+counter phrases 二通 / 八時 / 四十分, transparent compounds ○○部品 / ○○機能 / ○○鋼) — **curating a clean batch of 15 required scanning ~200 candidates**. Pure reconfirmation; standing recommendation unchanged (curator `clean_up_candidates_list.md` purge + this item's intake plausibility heuristic).

The **new** datum concerns the *good* lane, not the junk lane. Two of the run's "seen in entry" priority candidates — the highest-quality source in the pool — turned out to be **variant readings of entries that already exist**:

| Candidate | Duplicates | Note |
|---|---|---|
| 雪ぐ / すすぐ | **30089** | the source entry 06614 actually uses そそぐ |
| 裏面 / うらめん | **18245** | existing entry reads りめん |

Both would have been created as duplicate entries had the run not caught them by hand. The cause is that the comprehensive-polish **"seen in entry" logger records a surface+reading pair without checking the reading against existing entries** — `build/check_duplicate.py` and `manage_candidates.py check` exist precisely for this, but the *logging* side does not call them, so the dedupe burden falls on the (much later) new-entries run. **Sub-fix**: have the seen-in-entry capture path run the duplicate check at add time and either skip the candidate or annotate it (`possible variant reading of NNNNN`). This is cheaper and more reliable than catching it at creation time, and it protects the one candidate lane that still has signal. Note the second row also shows the check must compare **surface** as well as reading (裏面 exists under a *different* reading — a legitimate variant-reading question for the curator, not a straight duplicate), so the annotation form is preferable to a silent skip.
...
**Update 2026-07-25 (~2% precision on 17911–18345, with the truncation artifact now traced to the *prompt builder's context string* and 20 entries verified individually)**: A 2026-07-25 accuracy-review furigana screen over **17911–18345** flagged **48 entries with 1 genuine (~2%)**. The value of this measurement is the diagnosis, which is sharper than previous updates: the observing run verified the flagged entries against their JSON and found the screener's **per-pair context string is truncated by the prompt builder itself** — the reviewer's stated concern *quotes* a reading cut off mid-word and **sometimes carries a stray `)` where the wrapper's `}` was cut** (`{配布物|はいふぶ)`, `{茶店|さて)`, `{防腐剤|ぼうふざ)`), then flags that truncation as an "incomplete reading". Twenty entries were checked individually — **17917 / 17920 / 17928 / 17944 / 17945 / 17978 / 18012 / 18023 / 18057 / 18070 / 18148 / 18152 / 18161 / 18162 / 18183 / 18292 / 18297 / 18298 / 18312 / 18315** — and **all carry the full correct reading**. This single artifact accounts for **~37 of the 48 flags**.

The stray `)` is the diagnostic detail worth acting on: it proves the truncation happens **when the context window is built**, not when the model reads the entry, because a model misreading a correct wrapper would not invent an unbalanced delimiter. So the fix is entirely local to `review_runner.py`'s prompt construction — **send the pair/field untruncated (or truncate only at wrapper boundaries)** — and it would remove roughly three-quarters of all screening flags on any range, at zero model cost. Combined with [item 21](#21-chunk-the-reviewscreening-runners-to-fit-the-session-timeout)'s new ~8/min-vs-~29/min throughput measurement, this run is the strongest single argument in the file that **the furigana screener's cost is dominated by an artifact of our own prompt builder**, not by the review problem itself.
...
## 34. Verb-class mis-assignment detector (fabricated conjugation tables on genuine verbs)

**Source**: Routine v2 polish run, 2026-07-25 (06624 甘える, 09361 バックレる — both fixed in-run)

[Cleanup P6](../ideas/cleanup-backlog.md#priority-6-spurious-conjugation-tables-on-non-verb-entries) resolved conjugation tables on entries that are **not verbs**, and hardened `add_conjugations.py` with an exact-enum verb-POS guard. The guard cannot see the sibling defect: a **genuine verb carrying the wrong verb class**. **06624 甘える** (ichidan) was tagged `verb-godan` / `godan-ru`, so the generator produced an entire table of invented godan morphology — 甘えらない / 甘えります / 甘えった. Nothing in validation, CI, or the §4 model self-check flags it: the JSON is schema-valid, the POS is a legal verb POS, and the forms are internally consistent with the (wrong) class. Learners get a full page of forms that do not exist.

**Why the obvious heuristic is not enough**: scanning `verb-godan` entries whose reading ends in **-eru** returned **38 hits of which 36 were correct godan** — the well-known 帰る / 滑る / 蹴る / 返る family. ~5% precision is too noisy to sweep.

**Suggested detector (high precision)**: cross-check the entry's **own examples** against its declared class. Flag a `verb-godan` entry whose examples contain the **ichidan** past/て-form of its headword (`甘えた` / `甘えて` where the declared class predicts `甘えった` / `甘えって`), and symmetrically flag a `verb-ichidan` entry whose examples contain godan-only forms. This works because the example sentences and the conjugation table are produced by **different processes** (hand/model-written prose vs. `add_conjugations.py`), so a disagreement between them is strong evidence rather than a heuristic guess — it found the second real case (09361 バックレる, contradicted by its own バックレた) inside the 38-hit noise. Implementable as a read-only `--json` review queue like the other detectors; the fix once verified is simply `add_conjugations.py --force` after correcting `tags.pos` / `verb_class`.

**Scope**: unmeasured dictionary-wide; 2 confirmed cases, both fixed 2026-07-25. Worth running the example-cross-check once over all verb entries to size it — this is a *correctness* class (fabricated morphology), so even a handful justifies the detector.

## 35. Cross-reference `type` vocabulary: CLAUDE.md documents types the schema rejects

**Source**: Routine v2 polish run, 2026-07-25 (hit live — a `formality_variant` cross-reference failed validation)

`CLAUDE.md` (and the wording around the `cross-reference-entry` skill) refers to cross-reference types **`formality_variant`** and **`transitivity_pair`**, but `build/schema.json`'s enum allows only:

```
pair | synonym | antonym | keigo | related | see_also | contrast | homophone
```

Writing `formality_variant` therefore fails validation. This is a small, cheap inconsistency with an outsized cost pattern: a polishing session reads the documentation, writes the documented type, and only discovers the mismatch when validation fails mid-run — and the natural recovery (silently substituting `related`) **loses the semantic distinction** the documented type was there to carry.

**Two possible resolutions, and they are not equivalent:**

1. **Correct the docs** to the eight schema types. Cheapest; but `keigo` and `pair` already cover much of what `formality_variant` / `transitivity_pair` meant, so check first whether the documented names were simply informal aliases for them — if so, say that explicitly in CLAUDE.md rather than deleting the terms, since the concepts are real and appear throughout the wiki ([Verb Transitivity](../topics/verb-transitivity.md), [Register](../topics/register.md)).
2. **Add the types to the schema.** Only worth it if the distinction is one the *site* should render differently (a transitivity pair is arguably a distinct relation from a generic `pair`). This is a curator call, and it implies a migration of existing `pair`/`keigo` refs that mean these things.

Either way the docs and the enum must agree; a **single source of truth** (generate the CLAUDE.md list from `schema.json`, or lint the docs against the enum) prevents the drift from recurring — the same "documented check does not match the real check" failure mode as [item 11](#11-inline-link-target-id-resolution-gate-in-validatepy-or-pre-commitci).
