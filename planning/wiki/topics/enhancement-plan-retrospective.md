...
`check_consistency.py` produces a report. It is *not* currently a CI gate. The health dashboard shows 2,818 note-structure issues, 6,405 asymmetric references, etc. — numbers that drift rather than monotonically decrease. Making the checker a gate on *new* entries (while leaving historical entries as report-only) would prevent new debt.
...
From the 2026-04-09 plan's target table, compared against `make report` on 2026-05-02 (26,345 entries):
...
| Cross-reference symmetry | 98% | 42.2% | 6,405 asymmetric references; largest remaining gap |
...
| Examples per entry (avg) | ≥ 4 | 4.0 | Target reached; **105,220 total examples** |
