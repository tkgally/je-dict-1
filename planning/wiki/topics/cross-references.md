...
Over 10,500 cross-references exist across 25,100+ entries (roughly 0.42 cross-references per entry on average; `report.py` measures the symmetry rate at ~44.4% as of 2026-04-27). The cross-reference count surged by over 1,000 in the last week thanks to systematic `add_cross-references.md` sessions covering entries 10461-11370. Coverage is growing through:
...
The current ratio of ~0.42 cross-references per entry is growing but still below target. Most entries have zero or one cross-reference. A reasonable target might be an average of 1-2 cross-references per entry, which would mean 25,000-50,000 total. Priority should go to:
