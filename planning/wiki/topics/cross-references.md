...
Over 5,700 cross-references exist across 22,700+ entries (roughly 0.25 cross-references per entry on average). Coverage is growing through:
...
The current ratio of ~0.25 cross-references per entry is low. Most entries have zero or one cross-reference. A reasonable target might be an average of 1-2 cross-references per entry, which would mean 22,700-45,400 total. Priority should go to:
