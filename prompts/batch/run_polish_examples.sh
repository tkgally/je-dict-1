#!/bin/bash
# run_polish_examples.sh — Polish example sentences in batches
...
PROMPT="Read prompts/polish_example_sentences.md and follow the instructions."
LOG_FILE="prompts/batch/logs/polish_examples_$(date +%Y%m%d_%H%M%S).log"

mkdir -p prompts/batch/logs

echo "=== Polish Example Sentences Batch — $TOTAL_RUNS runs ===" | tee "$LOG_FILE"
...
for i in $(seq 1 "$TOTAL_RUNS"); do
    ...
    if claude -p "$PROMPT" --verbose 2>&1 | tee "$RUN_LOG"; then
        ...
        if ! git diff --quiet || ! git diff --cached --quiet || [ -n "$(git ls-files --others --exclude-standard)" ]; then
            git add -A
            git commit -m "Polish examples — batch run $i of $TOTAL_RUNS ($(date +%Y-%m-%d))"
