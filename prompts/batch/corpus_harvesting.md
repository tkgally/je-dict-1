...
- `batch_size`: Number of corpus entries to process (default: 150)

## Workflow

1. **Read the starting point**: Check `prompts/corpus_harvesting_next_entry_number.txt` to find where to start.

2. **Determine batch size**: Use the configured batch_size parameter. If not specified, process **150 entries**.

3. **Process each entry one by one**:
   - Read the word and its example sentence from `corpus_extracted_words.json`
   - The example sentence shows how the word appeared in a corpus — use it as reference
...
6. **After finishing the batch**:
   - Update `prompts/corpus_harvesting_next_entry_number.txt` with the next entry number
   - Run validation:
     ```bash
     make validate
     ```
...
7. **Commit** (do NOT push — the pipeline handles pushing):
   ```bash
   git add candidate_words.json prompts/corpus_harvesting_next_entry_number.txt
   git commit -m "Corpus harvesting: process entries N-M, add X candidates"
   ```
