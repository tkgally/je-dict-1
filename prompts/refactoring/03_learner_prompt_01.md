...
`vocabulary_tier` and `example_count` fields. The audit script can use these for
a quick pre-filter to identify entries likely below minimum counts. However, the
script still needs to load full entry files to check per-sense compliance (the
index only has total example_count, not per-sense breakdowns).
...
Optimization hint: Use the enriched entries_index.json to quickly identify
basic/core entries with low example_count before loading their full JSON files.
